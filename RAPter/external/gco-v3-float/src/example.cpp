...
//#define GCO_ENERGYTYPE float // test new version
...
Scalar smoothFn(int p1, int p2, int l1, int l2)
{
	if ( (l1-l2)*(l1-l2) <= 4 ) return((l1-l2)*(l1-l2));
	else return(4);
}

Scalar dataFn(int p, int l, void *data)
{
...
void GridGraph_DArraySArraySpatVarying(int width,int height,int num_pixels,int num_labels)
{
    ...
    Scalar *V = new Scalar[num_pixels];
    Scalar *H = new Scalar[num_pixels];
