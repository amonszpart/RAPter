...
bool
Plane::Init( MiscLib::Vector< Vec3f > const& samples )
{
    // Aron: this used to be < 6, but didn't work that way
    return samples.size() < 3 ? false
                              : Init( samples[0], samples[1], samples[2] );
}
