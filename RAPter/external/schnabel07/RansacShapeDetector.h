#ifndef SCHNABEL_RANSACSHAPEDETECTOR_HEADER
#define SCHNABEL_RANSACSHAPEDETECTOR_HEADER
...
class DLL_LINKAGE RansacShapeDetector
{
	public:
        ...
        size_t          Detect        ( PointCloud &pc
                                      ...
                                      , MiscLib::Vector< int > *outShapeIndex = NULL
                                      , std::vector< MiscLib::Vector< size_t > > *outIndices = NULL
                                      );
	...
	private:
        ...
        bool DrawSamplesStratified( const IndexedOctreeType             & oct
                                    , size_t                              numSamples
                                    , size_t                              depth
                                    , MiscLib::Vector< int >       const& shapeIndex
                                    , MiscLib::Vector< size_t >         * samples
                                    , IndexedOctreeType::CellType const** node ) const;
        ...
        float CandidateFailureProbability( float candidateSize, float numberOfPoints, float drawnCandidates, float levels ) const
		{
            return std::min( std::pow( 1.f - candidateSize / (numberOfPoints * levels * (1 << (m_reqSamples - 1)))
                                       , drawnCandidates )
                             , 1.f );
		}
...
#endif // SCHNABEL_RANSACSHAPEDETECTOR_HEADER
