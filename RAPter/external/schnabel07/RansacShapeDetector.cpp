...
RansacShapeDetector::~RansacShapeDetector()
{
    for ( ConstructorsType::iterator i = m_constructors.begin(), iend = m_constructors.end();
                                     i != iend;
                                   ++i )
        (*i)->Release();
}
...
//#       pragma omp for schedule(dynamic, 10) reduction(+:genCands)
        for ( int candIter = 0; candIter < 200; ++candIter )
        {
            ...
            if ( !DrawSamplesStratified( globalOctree
                                         , m_reqSamples
                                         , sampleLevel
                                         , scoreVisitorCopy.GetShapeIndex()
                                         , &samples
                                         , &node) )
                continue;
            ++genCands;
            ...
            const size_t           c = samples.size();
            MiscLib::Vector<Vec3f> samplePoints( samples.size() << 1 ); // points and normals
            ...
            for ( ConstructorsType::const_iterator it   = m_constructors.begin(),
                                                   iend = m_constructors.end();
                                                   it != iend;
                                                 ++it                )
            {
                if (     (*it)->RequiredSamples() > samples.size()
                     || !(shape = (*it)->Construct(samplePoints))  )
                {
                    continue;
                }
                ...
                {
                    //std::cout << "shape: "; shape->Serialize(&std::cout,false); std::cout << "\n";
                    //std::cout << "samplePoints[i]: " << samplePoints[i].print() << " samplePoints[i+c]: " << samplePoints[i+c].print() << std::endl;
                ...
                Vec3f p,n;
                shape->Normal(p,&n);
                //std::cout << "verified: " << n.print() << std::endl;
...
#               pragma omp critical
    ...
    *drawnCandidates += genCands;
    ...
    if ( !genCands )
    {
        std::cerr << "[" << __func__ << "]: " << "warning, no candidates generated...m_constructors.size(): " << m_constructors.size() << std::endl;
        throw std::runtime_error("No shape constructors to generate from");
    }
}
                                            ...
                                            float *maxForgottenCandidate, float *candidateFailProb) const
{
    if ( !candidates.size() )
    {
        std::cerr << "[" << __func__ << "]: " << "candidates.size(): " << candidates.size() << std::endl;
        return false;
    }
    ...
    if ( candidates.size()
         && candidates.back().ExpectedValue() < *maxForgottenCandidate )
    {
...
size_t
RansacShapeDetector::Detect( PointCloud &pc
                             , size_t beginIdx
                             , size_t endIdx
                             , MiscLib::Vector< std::pair< RefCountPtr< PrimitiveShape >, size_t > > *shapes
                             , MiscLib::Vector< int > *outShapeIndex
                             , std::vector< MiscLib::Vector< size_t > > *outIndices
                             )
{
    float x = abs(2.33f);
    std::cout << "abs(2.33f): " << x << std::endl;
    float x1 = std::abs(2.33f);
    std::cout << "std::abs(2.33f): " << x1 << std::endl;

    size_t pcSize = endIdx - beginIdx;
    std::cout << "[" << __func__ << "] pcSize: " << pcSize << std::endl;
    /*
     * Initialization part
     */
    ...
    ScorePrimitiveShapeVisitor<FlatNormalThreshPointCompatibilityFunc, ImmediateOctreeType> subsetScoreVisitor(      m_options.m_epsilon, m_options.m_normalThresh );
    ScorePrimitiveShapeVisitor<FlatNormalThreshPointCompatibilityFunc, IndexedOctreeType  > globalScoreVisitor(3.f * m_options.m_epsilon, m_options.m_normalThresh );
    ...
    {
        std::cout << "stratified subsets: " << i << "/" << octrees.size() << std::endl;
        --i;
        ...
        if ( i )
        {
            ...
            for ( size_t j = 0; j < subsetSize; ++j )
            {
                ...
                index += j * bucketSize;
                if ( index >= pcSize )  index = pcSize - 1;
                subsetIndices[j] = index + beginIdx;
            }
            // move all the indices to the end
            std::sort( subsetIndices.begin(), subsetIndices.end(), std::greater< size_t >() );
    ...
    MiscLib::Vector< double > sampleLevelProbability( globalOctTreeMaxNodeDepth + 1 );
    {
        for ( size_t i = 0; i != sampleLevelProbability.size(); ++i )
        {
            std::cout << "setting " << i << " to ";
            sampleLevelProbability[i] = 1.0 / sampleLevelProbability.size();
            std::cout << sampleLevelProbability[i] << std::endl;
        }
    }

    size_t drawnCandidates          = 0, // keep track of number of candidates that have been generated
           generatedCandidates      = 0,
           testedCandidates         = 0,
           tddFailCandidates        = 0,
           numTries                 = 0, // number of loops since last successful candidate
           numShapes                = 0, // number of shapes found since the
           numInvalid               = 0; // number of points that have been assigned
    float  maxForgottenCandidate    = 0.f; // the maximum size of a canidate that has been forgotten
    // last housekeeping
    // to a shape since the last housekeeping
    MiscLib::Vector< int > shapeIndex(pc.size(), -1); // maintains for every point the index of the shape it has been assigned to or -1 if the point is not assigned yet
    ...
    do
    {
        //std::cout << "[" << __func__ << "]: " << "stepping main while..." << std::endl;
        ...
        MiscLib::Vector< double                     > sampleLevelProbSum( sampleLevelProbability.size() );
        {
            ...
            if ( sampleLevelProbSum.size() )
            {
                for ( size_t i = 1; i < sampleLevelProbSum.size() - 1; ++i )
                    sampleLevelProbSum[i] = sampleLevelProbSum[i-1] + sampleLevelProbability[i];
        ...
        // generate candidates
        {
            ...
            do
            {
                //std::cout << "[" << __func__ << "]: " << "generating candidates..." << std::endl;
                ...
                cfp1 = CandidateFailureProbability( bestExpectedValue     , currentSize - numInvalid, drawnCandidates, globalOctTreeMaxNodeDepth );
                cfp2 = CandidateFailureProbability( m_options.m_minSupport, currentSize - numInvalid, drawnCandidates, globalOctTreeMaxNodeDepth);
//                std::cout << "cfp1(" << cfp1 << ") >? " << "m_options.m_probability(" << m_options.m_probability << ") && "
//                          << "cfp2(" << cfp2 << ") >? " << "m_options.m_probability(" << m_options.m_probability << ")\n";
                if ( cfp1 == 1. && cfp2 == 1. )
                {
                    std::cout << "cfps_eq_one: " << ++cfps_eq_one << std::endl;
                }
            }
            while ( (cfp1 > m_options.m_probability) && (cfp2 > m_options.m_probability) && (cfps_eq_one < 100) );
        }
        ...
        {
            //std::cout << "while findbestcand" << std::endl;
            if ( !foundCandidate )
            {
            ...
            if ( outIndices ) outIndices->push_back( *(candidates.back().Indices()) );
...
#               pragma parallel for schedule(static, 100)
    ...
    while ( CandidateFailureProbability(m_options.m_minSupport, currentSize - numInvalid, drawnCandidates, globalOctTreeMaxNodeDepth) > m_options.m_probability
    ...
    if ( numInvalid )
    {
        ...
        for ( size_t i = beginIdx; i < beginIdx + currentSize; ++i )
            if ( shapeIndex[i] < 0 )    shapeIndex[i] = begin++;
            else                        shapeIndex[i] = shapeIterators[shapeIndex[i]]++;
    ...
    for ( size_t i = 0; i < octrees.size(); ++i )   delete octrees[i];
    ...
    for(size_t i = shapes->size(); i != 0; --i)
    {
        if(shapes->at(i - 1).second == 0)
        {
            ...
            if ( outIndices ) outIndices->erase( outIndices->begin() + i - 1 );
        }
    }

    // added by Aron: return assignments
    if ( outShapeIndex ) *outShapeIndex = shapeIndex;
