...
startAt=0
mergeMult="1"        # Values: [0.5, 0.8, 1 ] Be conserva   tive with merge, multiply the scale with this. ( < 1 does not work well for some reason, parallel planes survive)
nbExtraIter=20          # Values: [ 1..20 ] iteration count. Default: 2.
reprPwMult=1           # Values: [ 1, 5 ,10]. Multiplies pw with this number when doing lvl2 (representatives)
use90=12                # Values: [ 6, 100 ] Controls, when to add a 90 generation step. If large, will be re-set by runscript to be after "adopt".
...
cand_anglediv="0.2";          # for 3D: "2.5";
# Multiply scale by this number to get the segmentation (regionGrowing) spatial distance
segmentScaleMultiplier="0.2"; # for 3D: "2.5"; 0.15 for room_n0.04, with scale=0.025
