...
#include "globfit2/optimization/problemSetup.hpp"     // everyPatchNeedsDirection()
...
{
    bool verbose = true;
    typedef typename PrimitiveContainerT::value_type::value_type _PrimitiveT;
    typedef typename PointContainerT::value_type                 _PointPrimitiveT;
    ...
    enum DATA_COST_MODE { ASSOC_BASED = 0, BAND_BASED = 1, INSTANCE_BASED = 2};
    ...
    // parse params
    {
        ...
        pcl::console::parse_argument( argc, argv, "--unary", weights(0));
        pcl::console::parse_argument( argc, argv, "--pw"   , weights(1));
        pcl::console::parse_argument( argc, argv, "--cmp"  , weights(2));
        ...
        // data_cost_mode
        {
            ...
            /**/ if ( !data_cost_mode_str.compare( "assoc"   ) ) data_cost_mode = ASSOC_BASED; // default
            else if ( !data_cost_mode_str.compare( "band"    ) ) data_cost_mode = BAND_BASED;  // banded
            else if ( !data_cost_mode_str.compare( "instance") ) data_cost_mode = INSTANCE_BASED;
            else std::cerr << "[" << __func__ << "]: " << "could NOT parse data mode, assuming " << (int)data_cost_mode << std::endl;
        }
        ...
        // constr_mode
        {
            ...
            /**/ if ( !constr_mode_str.compare( "patch" ) ) constr_mode = PATCH_WISE; // default
            else if ( !constr_mode_str.compare( "point" ) ) constr_mode = POINT_WISE; // banded
        ...
        std::cerr << "[" << __func__ << "]: " << "Usage:\t gurobi_opt --formulate\n"
                  ...
                  << " [--data-mode *" << (int)data_cost_mode << "* (assoc | band | instance) ]\n"
                  << " [--constr-mode *" << (int)constr_mode << "* (patch | point | hybrid ) ]\n"
                  << " [--srand " << srand_val << "]\n"
                  << " [--rod " << problem_rel_path << "]\t\tRelative output path of the output matrix files\n"
                  << " [--patch-pop-limit " << patch_pop_limit << "]\n"
                  << std::endl;
    ...
    AbstractPrimitivePrimitiveEnergyFunctor<Scalar,_PrimitiveT> *primPrimDistFunctor = NULL;
    {
        ...
        // parse cost function
        {
                 if ( !cost_string.compare("cexp") )    primPrimDistFunctor = new CExpPrimitivePrimitiveEnergyFunctor<Scalar,_PrimitiveT>( angles );
            else if ( !cost_string.compare("sqrt") )    primPrimDistFunctor = new SqrtPrimitivePrimitiveEnergyFunctor<Scalar,_PrimitiveT>( angles );
                ...
                for ( size_t lid1 = 0; lid1 != prims[lid].size(); ++lid1 )
                {
                    for ( size_t olid = 0; olid != prims.size(); ++olid )
                    {
                        for ( size_t olid1 = 0; olid1 != prims[olid].size(); ++olid1 )
                        {
                            ...
                            Scalar dist = primPrimDistFunctor->eval( prims[lid][lid1], prims[olid][olid1] );
