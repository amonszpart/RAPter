...
$WORKSPACE/globOpt/globOpt/build/Release/bin/toGlobFit --planes --prims patches.csv --cloud cloud.ply -a points_primitives.csv --scale $scale
$WORKSPACE/3rdparty/globfit/build/bin/globfit_release -i segments.globfit -v -o 3.0 -g 3.0 -a 0.1 -p $scale -l $scale -r $scale
$WORKSPACE/globOpt/globOpt/build/Release/bin/toGlobFit --from segments_oa.globfit --planes --prims patches.csv --cloud cloud.ply -a points_primitives.csv --scale $scale

../../globOptVis --show3D --scale $scale --pop-limit $poplimit -p primitives_it24.bonmin.csv -a points_primitives_it25.csv --title "GlobOpt" $visdefparam --paral-colours --no-rel &
