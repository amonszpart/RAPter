...
namespace rapter
{
    ...
    int fromGlobFit( int argc, char **argv )
    {
        ...
        typedef typename rapter::containers::PrimitiveContainer<PrimitiveT> IterablePrimitiveContainer;
        ...
        if ( rapter::console::parse_argument( argc, argv, "--from", primitivesPath ) < 0 )
        {
            std::cerr << "[" << __func__ << "]: " << "need --from primitivesPath to exist!" << std::endl;
            return EXIT_FAILURE;
        }
        std::string inputSegmentsPath; // the original ransac input
        if (    (rapter::console::parse_argument( argc, argv, "-p"     , inputSegmentsPath) < 0)
                && (rapter::console::parse_argument( argc, argv, "--prims", inputSegmentsPath) < 0)
        ...
        if (    (rapter::console::parse_argument( argc, argv, "-s"     , scale) < 0)
             && (rapter::console::parse_argument( argc, argv, "--scale", scale) < 0)
                )
        {
        ...
        rapter::console::parse_argument( argc, argv, "-o", outName);
        ...
        err = rapter::io::readPrimitives<PrimitiveT, InnerPrimitiveContainerT>( segmentsVector, inputSegmentsPath, &segments );
        if ( EXIT_SUCCESS != err )
        {
...
//                      << "\tfrom line " << line << std::endl;
        ...
        while ( inStream && numPrimitives )
        {
            ...
            else if (primitiveType == "plane")
            {
                ...
                rapter::GidT gid = segmentsIt->second[0].getTag(PrimitiveT::TAGS::GID);
                rapter::DidT did = segmentsIt->second[0].getTag(PrimitiveT::TAGS::DIR_GID );
...
//            }
        ...
        rapter::io::savePrimitives<PrimitiveT,typename InnerPrimitiveContainerT::const_iterator>( primitives, ssPrims.str() );
    ...
    int toGlobFit( int argc, char **argv )
    {
        ...
        std::string timeStamp = rapter::util::timestamp2Str();
        std::map< rapter::PidT, std::vector<rapter::PidT> > primitivesPoints;
