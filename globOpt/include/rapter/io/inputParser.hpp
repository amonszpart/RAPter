...
#ifndef RAPTER_INPUTPARSER_HPP
#define RAPTER_INPUTPARSER_HPP
...
inline std::string parseAssocPath( int argc, char** argv )
{
    ...
    if ( rapter::console::parse_argument( argc, argv, "-a"     , input_prims_path) < 0 )
        rapter::console::parse_argument( argc, argv, "--assoc", input_prims_path);
...
inline std::string parsePrimitivesPath( int argc, char** argv )
{
    ...
    if ( rapter::console::parse_argument( argc, argv, "-p"     , input_prims_path) < 0 )
        rapter::console::parse_argument( argc, argv, "--prims", input_prims_path);
                     ...
                     , bool                      read_assoc = true )
{
    ...
    if ( EXIT_SUCCESS == err )
    {
        ...
        err = rapter::io::readPrimitives<PrimitiveT, _InnerPrimitiveContainerT>( initial_primitives, input_prims_path, &patches );
        if ( EXIT_SUCCESS == err )
    ...
    if ( read_assoc )
    {
        ...
        rapter::io::readAssociations( points_primitives, associations_path, NULL );
